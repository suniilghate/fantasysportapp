...
$(document).on('click', '.player-lobby', function (event) {
  ...
  let matchtId = $(this).attr("data-id");
  let contestId = $(this).attr("data-id-1")
  let entryFee = $(this).attr("data-id-2");
...
$(document).on('submit', '#selectLobyPlayers', function (event) {
  ...
  $("input:checked").each(function() {
      myCheckboxes.push({'player_id' : $(this).val(), 'team_id': $(this).attr('data-id'), 'type': $(this).attr('data-id-1'), 'name': $(this).attr('data-id-2'), 'team': $(this).attr('data-id-3')});
  });
  let flagCheck = true;
  var team1Players = [];
  var team2Players = [];
...
{
    if(cnt>=Start && cnt<=End) return true;
    else return false;
}